...
class Workout1 extends MX_Controller
{
    ...
    public function mulaitest() {
        if (!empty($this->session->userdata['id_latihan'])) {
            ...
            $query = $this->Mworkout1->get_soal($id, $kesulitan);
    ...
    $level = $this->Mworkout->levelLatihan($id_latihan)[0]->level;
    $result = $this->Mworkout->jawabansoal($id);
    $benar = 0;
    $salah = 0;
    $kosong = 0;
    $koreksi = array();
...
//
    $result = $this->Mworkout->inputreport($hasil);
    $this->Mworkout->updateLatihan($id_latihan);
    ...
    $data = array(
        'judul_halaman' => 'Neon - Daftar Workout',
        'judul_header' => 'History Workout',
        );

    $konten = 'modules/workout/views/v-daftar-test.php';
    ...
    // $data['report'] = $this->Mworkout->get_report($this->session->userdata['USERNAME']);
    // $data['latihan'] = $this->Mworkout->get_latihan($this->session->userdata['USERNAME']);
